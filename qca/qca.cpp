...
static QList<ProviderItem*> providerList;
static bool qca_init = false;

static bool plugin_have(const QString &fname)
{
	for(int n = 0; n < providerList.size(); ++n) {
		ProviderItem *i = providerList[n];
		if(i->fname == fname)
			return true;
	}
	return false;
...
{
	QByteArray out(str.length() / 2, 0);
	int at = 0;
	for(int n = 0; n + 1 < (int)str.length(); n += 2) {
		uchar a = str[n].toLatin1();
		uchar b = str[n+1].toLatin1();
...
{
	QByteArray buf(d->c->blockSize(), 0);
...
bool RSAKey::fromPEM(const QString &str)
{
	...
	return d->c->createFromPEM(cs.data(), cs.size());
}
...
{
	QList<QCA_CertProperty> list = d->c->subject();
	CertProperties props;
	for(QList<QCA_CertProperty>::ConstIterator it = list.begin(); it != list.end(); ++it)
		props[(*it).var] = (*it).val;
	return props;
}
...
{
	QList<QCA_CertProperty> list = d->c->issuer();
	CertProperties props;
	for(QList<QCA_CertProperty>::ConstIterator it = list.begin(); it != list.end(); ++it)
		props[(*it).var] = (*it).val;
	return props;
...
void TLS::setCertificateStore(const QList<Cert*> &store)
{
	// convert the cert list into a context list
	d->store.clear();
	for(int n = 0; n < store.size(); ++n)
		d->store.append(store[n]->d->c);
}
