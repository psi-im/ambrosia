...
namespace XMPP
{
	...
	public:
		...
		virtual void connectToServer(const QString &server, const QString &mode)=0;
	...
	public:
		...
		void connectToServer(const QString &server, const QString &mode);
	...
	public:
		...
		Stanza();
		Stanza(Kind k, const Jid &to="", const QString &type="", const QString &id="");
		...
		void setBaseNS(const QString &ns);
	...
	public:
		...
		ClientStream(const QString &host, const QString &defRealm, ByteStream *bs, QCA::TLS *tls=0, bool sslnow=false, bool s2s=false, QObject *parent=0); // server
		~ClientStream();

		Jid jid() const;
		void connectToServer(const Jid &jid, bool auth=true);
		void connectToServerAsServer(const Jid &to, const Jid &from, const QString &key);
		void connectToServerAsServerVerify(const Jid &to, const Jid &from, const QString &id, const QString &key);
		void accept(); // server
		bool isActive() const;
		...
		void dialbackRequestGrant(const Jid &to, const Jid &from, bool ok);
		void dialbackVerifyRequestGrant(const Jid &to, const Jid &from, const QString &id, bool ok);
	...
	signals:
		...
		void authenticated();
		void warning(int);
		void dialbackRequest(const Jid &to, const Jid &from, const QString &key);
		void dialbackResult(const Jid &from, bool ok);
		void dialbackVerifyRequest(const Jid &to, const Jid &from, const QString &id, const QString &key);
		void dialbackVerifyResult(const Jid &from, bool ok);
