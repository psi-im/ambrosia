...
static QString lookupUserPassword(const QString &user)
{
	QFile f("userdb");
...
void BasicProtocol::extractStreamError(const QDomElement &e)
{
	...
	if(errCond != -1) {
		...
		for(uint n = 0; n < (uint)nl.count(); ++n) {
...
void CoreProtocol::startDialbackVerifyOut(const QString &_to, const QString &_from)
{
	server = true;
	dialback = true;
...
void CoreProtocol::sendDialbackResultRequest(const Jid &to, const Jid &from, const QString &key)
{
	DBItem i;
	i.type = DBItem::ResultRequest;
	i.to = to;
	i.from = from;
	i.key = key;
	dbrequests += i;
...
void CoreProtocol::sendDialbackResultGrant(const Jid &to, const Jid &from, bool ok)
{
	DBItem i;
	i.type = DBItem::ResultGrant;
	i.to = to;
	i.from = from;
	i.ok = ok;
	dbrequests += i;
...
void CoreProtocol::sendDialbackVerifyRequest(const Jid &to, const Jid &from, const QString &id, const QString &key)
{
	DBItem i;
	i.type = DBItem::VerifyRequest;
	i.to = to;
	i.from = from;
	i.id = id;
	i.key = key;
...
void CoreProtocol::sendDialbackVerifyGrant(const Jid &to, const Jid &from, const QString &id, bool ok)
{
	DBItem i;
	i.type = DBItem::VerifyGrant;
	i.to = to;
	i.from = from;
	i.id = id;
	i.ok = ok;
...
bool CoreProtocol::isValidStanza(const QDomElement &e) const
{
	...
	if((e.namespaceURI() == NS_SERVER || e.namespaceURI() == NS_CLIENT) && (s == "message" || s == "presence" || s == "iq"))
		return true;
	else
		return false;
}
			...
			if(isReady()) {
				if(isValidStanza(e)) {
					if(isIncoming()) {
						...
						from = from.domain();
						printf("verifying sender: [%s]\n", from.full().toLatin1().data());
						bool valid = false;
						for(int n = 0; n < dbvalidated.size(); ++n) {
							DBItem &i = dbvalidated[n];
							Jid j = i.to.domain();
							printf("  with: [%s]\n", j.full().toLatin1().data());
							if(j.compare(from)) {
								valid = true;
								break;
							}
						}
						// disconnect if stanza is from unverified sender
						if(!valid)
							return error(0);
						...
						// ignore packets from receiving servers
					}
				}
			}
		}
...
bool CoreProtocol::normalStep(const QDomElement &e)
{
	...
	// server
		...
		if(e.namespaceURI() == NS_CLIENT && e.tagName() == "iq") {
			...
			if(!q.isNull())
			{
				...
				if(type == "get")
				{
					QDomElement u = e.elementsByTagNameNS("jabber:iq:auth", "username").item(0).toElement();
					QString user;
					if(!u.isNull())
						user = u.text();
					...
					r.setAttribute("type", "result");
					r.setAttribute("id", e.attribute("id"));
					QDomElement rq = doc.createElementNS("jabber:iq:auth", "query");
					{
						QDomElement u = doc.createElement("username");
						u.appendChild(doc.createTextNode(user));
						rq.appendChild(u);
						rq.appendChild(doc.createElement("digest"));
						rq.appendChild(doc.createElement("resource"));
					}
					r.appendChild(rq);
					...
					user = q.elementsByTagName("username").item(0).toElement().text();
					resource = q.elementsByTagName("resource").item(0).toElement().text();
					QString digest = q.elementsByTagName("digest").item(0).toElement().text();

					QString user_pass = lookupUserPassword(user);
					QByteArray cs = id.toUtf8() + user_pass.toUtf8();
					QString our_digest = QCA::SHA1::hashToString(cs);
					if(user.isEmpty() || digest != our_digest) {
						printf("bad login\n");
						return error(0);
					}
	...
	else if(step == HandleBindSuccess) {
		return loginComplete();
	}
