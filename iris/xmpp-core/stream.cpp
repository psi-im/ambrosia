...
static QDomElement changeNS(const QDomElement &e, const QString &oldns, const QString &newns)
{
	...
	QString ns;
	if(e.namespaceURI() == oldns)
		ns = newns;
	else
		ns = e.namespaceURI();
	QDomElement i = e.ownerDocument().createElementNS(ns, qName);
	uint x;
	...
	for(x = 0; x < (uint)nl.count(); ++x) {
		QDomNode n = nl.item(x);
		if(n.isElement())
			i.appendChild(changeNS(n.toElement(), oldns, newns));
		else
...
Stanza::Stanza(Kind k, const Jid &to, const QString &type, const QString &id)
{
	d = new Private;
	...
	d->baseNS = "jabber:client";
	d->e = d->doc.createElementNS(d->baseNS, Private::kindToString(kind));
	if(to.isValid())
		setTo(to);
	if(!type.isEmpty())
		setType(type);
...
{
	return d->doc.createElementNS(ns, tagName);
}

QDomElement Stanza::createTextElement(const QString &ns, const QString &tagName, const QString &text)
{
	QDomElement e = d->doc.createElementNS(ns, tagName);
	e.appendChild(d->doc.createTextNode(text));
	return e;
}
...
void Stanza::setBaseNS(const QString &ns)
{
	// if same, do nothing
	if(d->baseNS == ns)
		return;

	printf("changing ns: [%s] -> [%s]\n", d->baseNS.toLatin1().data(), ns.toLatin1().data());
	QString oldns = d->baseNS;
	d->baseNS = ns;
	d->e = changeNS(d->e, oldns, d->baseNS);
}

Stanza::Error Stanza::error() const
{
	Error err;
	QDomElement e = d->e.elementsByTagNameNS(d->baseNS, "error").item(0).toElement();
	if(e.isNull())
		return err;
...
void Stanza::setError(const Error &err)
{
	...
	QDomElement errElem = d->e.elementsByTagNameNS(d->baseNS, "error").item(0).toElement();
	if(errElem.isNull()) {
		errElem = d->e.ownerDocument().createElementNS(d->baseNS, "error");
	...
	//else {
		...
		errElem.appendChild(d->e.ownerDocument().createElementNS(d->baseNS, scond));
	//}
	...
	//}
	//else {
		QDomElement te = d->e.ownerDocument().createElementNS(d->baseNS, "text");
...
ClientStream::ClientStream(const QString &host, const QString &defRealm, ByteStream *bs, QCA::TLS *tls, bool sslnow, bool s2s, QObject *parent)
:Stream(parent)
{
	d = new Private;
	d->mode = Server;
...
void ClientStream::connectToServer(const Jid &jid, bool auth)
{
	...
	d->conn->connectToServer(d->server, "xmpp-client");
}

void ClientStream::connectToServerAsServer(const Jid &to, const Jid &from, const QString &key)
{
	reset(true);
	d->state = Connecting;
	//d->jid = jid;
	...
	d->conn->connectToServer(d->server, "xmpp-server");
}

void ClientStream::connectToServerAsServerVerify(const Jid &to, const Jid &from, const QString &id, const QString &key)
{
	reset(true);
	d->state = Connecting;
	//d->jid = jid;
...
void ClientStream::accept()
{
	...
	if(d->sslnow) {
		...
		ByteStream::appendArray(&d->spare, a);

		printf("starting tls server [spare=%d]\n", d->spare.size());
		...
		d->using_tls = true;
		d->ss->startTLSServer(d->tls, d->spare);
		d->spare.resize(0);
	}
	else
		processNext();
...
void ClientStream::dialbackRequestGrant(const Jid &to, const Jid &from, bool ok)
{
	d->srv.sendDialbackResultGrant(to, from, ok);
	processNext();
}

void ClientStream::dialbackVerifyRequestGrant(const Jid &to, const Jid &from, const QString &id, bool ok)
{
	d->srv.sendDialbackVerifyGrant(to, from, id, ok);
	processNext();
}
...
void ClientStream::cr_connected()
{
	...
	if(d->s2s) {
		if(d->s2s_verify) {
			d->client.startDialbackVerifyOut(QString(), QString());
		}
		else {
			d->client.startDialbackOut(QString(), QString());
...
void ClientStream::sasl_authCheck(const QString &user, const QString &authzid)
{
#ifdef XMPP_DEBUG
		...
		switch(event) {
			...
			case CoreProtocol::ERecvOpen: {
				...
				if(d->s2s) {
					...
					str = QCA::SHA1::hashToString(str + d->srv.from.toUtf8()).toUtf8();
			...
			case CoreProtocol::EStanzaReady: {
				...
				Stanza s = createStanza(d->srv.recvStanza());
				if(s.isNull()) {
					printf("unable to create stanza\n");
					break;
				}
				d->in.append(new Stanza(s));
				break;
			}
			case CoreProtocol::EDBRequest: {
#ifdef XMPP_DEBUG
				printf("db req: [%s]\n", d->srv.dbkey.toLatin1().data());
#endif
				dialbackRequest(d->srv.dbto, d->srv.dbfrom, d->srv.dbkey);
				break;
			}
			case CoreProtocol::EDBVerify: {
				printf("DBVerify (server)\n");
				dialbackVerifyRequest(d->srv.dbto, d->srv.dbfrom, d->srv.dbid, d->srv.dbkey);
				break;
			}
		}
	}
		...
		switch(event) {
			...
			case CoreProtocol::EReady: {
				...
				if(d->s2s) {
					d->state = Active;
					if(d->s2s_verify) {
						d->client.sendDialbackVerifyRequest(d->jid, d->dbfrom, d->dbid, d->dbkey);
					}
					else {
						d->client.sendDialbackResultRequest(d->jid, d->dbfrom, d->dbkey);
					}
					break;
				}
			...
			/*case CoreProtocol::EDBVerify: {
				printf("DBVerify (client)\n");
				dialbackVerifyRequest(d->client.dbto, d->client.dbfrom, d->client.dbkey);
				break;
			}*/
			case CoreProtocol::EDBRequestResult: {
				printf("DBRequestResult\n");
				dialbackResult(d->client.dbfrom, d->client.dbok);
				break;
			}
			case CoreProtocol::EDBVerifyResult: {
				printf("DBVerifyResult\n");
				dialbackVerifyResult(d->client.dbfrom, d->client.dbok);
				break;
			}
		}
	}
