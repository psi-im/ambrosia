...
public:
	...
	{
		// hack for ip addresses
		QHostAddress ip(QString::fromLatin1(name));
		if(!ip.isNull())
		{
			Item *i = new Item;
			i->id = next_id++;
			...
			QMetaObject::invokeMethod(this, "do_ip", Qt::QueuedConnection, Q_ARG(int, i->id), Q_ARG(QHostAddress, ip));
			return i->id;
		}
...
private slots:
	...
	void do_ip(int id, const QHostAddress &ip)
	{
		Item *i = 0;
		for(int n = 0; n < items.count(); ++n)
		{
		...
		QList<NameRecord> out;
		NameRecord rec("example.com", 7200);
		rec.setAddress(ip);
		out += rec;
		emit resolve_resultsReady(id, out);
	}
