...
{
    QByteArray a( 16383, 0 ); // large enough for anything, one suspects
...
void Q3DnsManager::transmitQuery( int i )
{
    ...
    QByteArray p( 12 + q->l.length() + 2 + 4, 0 );
    if ( p.size() > 500 )
    ...
    q->start( q->step < ns->count() ? 800 : 1500 );
}
...
Q3PtrList<Q3DnsRR> * Q3DnsDomain::cached( const Q3Dns * r )
{
	...
	if ( !nxdomain ) {
	    ...
	    if ( q == m->queries.size() && ( s.indexOf( '.' ) >= 0 ||
...
Q3DnsSocket::Q3DnsSocket( QObject * parent, const char * )
    : QObject( parent )
{
    // nothing
}
...
QString Q3Dns::toInAddrArpaDomain( const QHostAddress &address )
{
	...
	Q_UINT32 i = address.toIPv4Address();
...
void Q3Dns::doResInit()
{
	...
	while ( !stream.atEnd() ) {
		...
		if ( address->setAddress( QString(line[1]) ) ) {
		    ...
		    if ( is_ipv4(*address) || ipv6support )
			ns->append( address );
		} else {
		    delete address;
		}
	    } else if ( type == "search" ) {
		QStringList srch = list[1].split(" ", QString::SkipEmptyParts );
		for ( QStringList::Iterator i = srch.begin(); i != srch.end(); ++i )
		    domains->append( (*i).toLower().toLocal8Bit() );

	    } else if ( type == "domain" ) {
		domains->append( list[1].toLower().toLocal8Bit() );
	    }
	}
    }
...
#  if defined(MAXDFLSRCH)
	for( i=0; i < MAXDFLSRCH; i++ ) {
		...
		domains->append( QString::fromLatin1( res.dnsrch[i] ).toLower().toLocal8Bit() );
	    else
		break;
	}
#  endif
	if ( *res.defdname )
	    domains->append( QString::fromLatin1( res.defdname ).toLower().toLocal8Bit() );
#else
	q3dns_res_init();
	int i;
	// find the name servers to use
...
#  endif
	...
	while( !i.atEnd() ) {
	    ...
	    QHostAddress a;
	    a.setAddress( ip );
	    if ( ( is_ipv4(a) || is_ipv6(a) ) && !a.isNull() ) {
		bool first = true;
		line = line.mid( n+1 );
		n = 0;
		while( (int) n < line.length() && !line[(int)n].isSpace() )
