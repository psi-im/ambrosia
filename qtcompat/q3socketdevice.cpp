...
//	const QFile *f = qt_cast<const QFile *>(that);
//	if (f) return (int) f->error();
#endif
	return that->isOpen() ? 0 /* IO_Ok */ : 8 /* IO_UnspecifiedError */;
}

void iodev_resetStatus(QIODevice *)
{
#if !defined(QT_NO_QOBJECT)
//	QFile *f = qt_cast<QFile *>(that);
//	if (f) f->unsetError();
#endif
}
...
{
	return h.protocol() == QAbstractSocket::IPv4Protocol || h.protocol() == QAbstractSocket::UnknownNetworkLayerProtocol;
}

bool is_ipv6(const QHostAddress &h)
{
	return h.protocol() == QAbstractSocket::IPv6Protocol;
}
...
*/
Offset Q3SocketDevice::size() const
{
    return 0;
}
